...
def calculate_distance_fft(feature1, feature2):
    return np.linalg.norm(feature1 - feature2) / feature2.shape[0]
...
def inference(testfile, u_fft, e_fft, o_fft, a_fft, i_fft):
...
def inference_on_test(u_fft, e_fft, o_fft, a_fft, i_fft):
    vowel_files = []
    ...
    for file in vowel_files:
        predict, label = inference(file, u_fft, e_fft, o_fft, a_fft, i_fft)
